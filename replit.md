...
- **Connection Recovery System**: Addresses Supabase Web Lock deadlock bug (GitHub Issue #1594) that causes infinite hangs when tabs sleep/background. Recovery flow: (1) noOpLock bypasses Web Lock API, (2) detectSessionInUrl: false for webview compatibility, (3) Gatekeeper enforces 3-retry limit with global counter reset on success, (4) After max retries, SupabaseManager.reinitialize() clears state and creates fresh client, (5) Auto-retries load after successful reinit, (6) Shows Reconnect button only if reinit fails.
